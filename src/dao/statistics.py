...
class StatsDAO:
    ...
    def getMessagesPerDay(self):
        cursor = self.conn.cursor()
        query = "select mupload_date, count(mupload_date) from messages where mupload_date between '01/01/2017' and '12/12/2021' group by mupload_Date order by mupload_date;"
        cursor.execute(query)
        result = []
        for row in cursor:
            result.append(row)
