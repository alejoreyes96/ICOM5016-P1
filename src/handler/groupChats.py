...
class ChatHandler:
    ...
    def reactToMessageInGroupChatByUserIdAndGroupChatIdAndMessageId(self, userid, groupchatid, messageid, form):
        ...
        if len(form) != 1:
            ...
            if rtype is not None and rupload_date:
                rid = dao.reactToMessageInGroupChatByUserIdAndGroupChatIdAndMessageId(userid, groupchatid, messageid,
                                                                                      rtype)
    ...
    def replyToMessageInGroupChatByUserIdAndGroupChatIdAndMessageId(self, userid, groupchatid, messageid, json):
        ...
        if len(json) != 6:
            ...
            if rptype and rpupload_date:
                rpid = dao.replyToMessageInGroupChatByUserIdAndGroupChatIdAndMessageId(userid, groupchatid, messageid,
                                                                                       rpreply,
                                                                                       rpsize, rplength, rptype, rppath)
                result = self.build_reply_attributes(rpid, rpreply, rpupload_date, rpsize, rplength, rppath, rptype,
                                                     messageid, userid)
    ...
    def insertMessage(self, userid, groupchatid, json):
        ...
        result = self.build_messages_attributes(mid, mmessage, mupload_date, msize, mlength, mtype, mpath, mhashtag,
                                                uid)
    ...
    if not dao.getMessageFromGroupChatById(userid, groupchatid, mid):
        ...
        if len(json) != 7:
            ...
            if mmessage and msize and mlength and mtype and mpath:
                ...
                result = self.build_messages_attributes(mid, mmessage, mupload_date, msize, mlength, mtype, mpath,
                                                        mhashtag, uid)
                for value in mhashtag:
    ...
    if not dao.getReplyByIdOnly(rpid):
        ...
        if len(json) != 7:
            ...
            if rpreply:
                ...
                result = self.build_reply_attributes(rpid, rpupdate_date, rpreply, rpsize, rplength, rptype, rppath,
                                                     messageid, uid)
                for value in rphashtag:
                    if dao.getHashtagByHashtag(value) is None:
                        entry = dao.insertHashtagAndContainsFromReply(rpid, value)
