...
CREATE TABLE Group_Chats(gid serial primary key,gname varchar(20),
gcreation_date varchar(10),gpicture_id_path varchar(20),
huid integer references Human(huid));

CREATE TABLE IsMember(uid integer references Users(uid),
gid integer references Group_Chats(gid),primary key(uid,gid));

CREATE TABLE Messages(mid serial primary key,uid integer references Users(uid),
mupload_date varchar(10),msize integer,mmessage varchar(100), mmedia_path varchar(50),
mlength integer,mtype varchar(50));

CREATE TABLE Hashtags(hid serial primary key,hhashtag varchar(50));

CREATE TABLE Posted_To(mid integer references Messages(mid), gid integer references
Group_Chats(gid), primary key(mid,gid));
...
CREATE TABLE Contains(cid serial primary key,mid integer references Messages(mid),
hid integer references Hashtags(hid), rpid integer references Replies(rpid));
